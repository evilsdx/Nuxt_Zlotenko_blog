...
const fetchCategory = async () => {
  try {
    const category = await $fetch(`${config.public.apiBase}/blog/categories/${id}`)
    form.value = {
    ...
    errorMsg.value = 'Помилка завантаження категорії'
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    errorMsg.value = 'Помилка оновлення категорії'
  } finally {
